...
| 字段特征 | 自动添加的校验 |
|---------|--------------|
| 包含 `email`, `mail` | `pattern`（邮箱正则） |
| 包含 `phone`, `mobile` | `pattern`（手机号正则，支持+86区号） |
| 包含 `url`, `link` | `pattern`（URL正则） |
...
| 包含 `size`, `limit` | `minimum=1, maximum=100` |
...
- 缺少 `operationId` → 根据 summary 自动生成
