...
点击"自动修复"后，会弹出 **Diff 预览窗口**，清晰展示修改内容：

- **左右对齐**：修复前/修复后两列完全对齐
- **颜色区分**：绿色=新增，红色=删除，灰色=空行占位
- **同步滚动**：滚动一边，另一边同步滚动
- **统计显示**：显示添加/删除行数
- **确认/取消**：可选择应用修复或取消

![Diff 预览](docs/images/web-ui-diff-2-preview.png)

*Diff 预览窗口 - 展示 YAML 修改前后对比*
...
测试套件包含 **34 个测试用例**，分为 4 大类：

#### 测试结果截图

![测试结果](docs/images/test-results.png)

*测试结果 - 包含 Issue #2, #4 修复验证*
...
| 分类 | 用例数 | 说明 |
|------|--------|------|
| **错误检测** | 13 | YAML 结构错误、path // 校验、Swagger/OpenAPI 格式检测 |
